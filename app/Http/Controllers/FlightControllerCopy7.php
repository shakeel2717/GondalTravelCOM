...
class FlightControllerCopy7 extends Controller
{
    private $ApiUrl;
    private $ApiLive = true;
    private $BookingLive = false;
    ...
    {
        // $this->get_pnr_record("eJzTd9cPCfR0DgsDAAutAoM%3D");
        // exit;
        // $this->delete_pnr_record("eJzTd9cPCfR0DgsDAAutAoM%3D");
        // exit;
        //dd($request);exit;
        $access_token = $this->getApi();
    ...
    private function get_pnr_record($id)
    {
        ...
        //$fields = http_build_query($pnr_id);
        $url = $flight . '/' . $pnr_id;
        //echo $url;exit;
        $headers = array('Authorization' => 'Bearer ' . $access_token);
        $requests_response = Requests::get($url, $headers);
