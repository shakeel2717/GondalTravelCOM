...
use App\Mail\ticket as MailTicket;
...
class AdminControllerCopy extends Controller
{
    private $ApiUrl;
    private $ApiLive;
    private $BookingLive;
    ...
    public function allUserDataHistory(Request $request)
    {
        ...
        if ($request->ajax()) {
            ...
            foreach ($data as $d) {
                ...
                $d->date =  (string)date('d-M-Y H:i', strtotime($d->created_at));
                $d->departure_date =  (string)date('d-M-Y H:i', strtotime($d->departure_date));
                $d->return_date =  (string)date('d-M-Y H:i', strtotime($d->return_date));
            }
    ...
    public function editbooking($pnr)
    { $uploadtickets = ticketupload::where('pnr_number', $pnr)->first();
            $ticket = Ticket::where('prn_no', $pnr)->first();
            if ($ticket->format == 'db') {
                $data = json_decode($ticket->details, true);
                $passenger = Passenger::where('ticket_id', $ticket->id)->get();
                return view('admin.editbooking', ['ticket2' => $ticket, 'data2' => $data, 'passengers2' => $passenger, 'originalticket' => $uploadtickets, 'error' => false]);
            } else {
                $data = json_decode($ticket->details, true);
                $passenger = Passenger::where('ticket_id', $ticket->id)->get();
                return view('admin.editbooking', ['ticket' => $ticket, 'data' => $data, 'passengers' => $passenger, 'originalticket' => $uploadtickets, 'error' => false]);
            }
    ...
    public function updatebooking(Request $request,$id)
    {
        $request->validate([
            ...
            'admin_profit' => 'required|numeric|min:0|max:100',
            'payment_iata' => 'required|numeric',
            'issued_from' => 'required',
            'ticket_status' => 'required',
        ]);
        $ticket = Ticket::find($id);
        $ticket->prn_no = $request->input("prn_no");
        $ticket->departure_date=$request->input("departure_date");
        $ticket->company = $request->input("company");
        $ticket->ticket_num = $request->input("ticket_num");
        $ticket->admin_price = $request->input("admin_price");
        $ticket->admin_profit =  $ticket->amount - $request->input("admin_price");
        $ticket->reminder= $request->input("reminder");
        // dd( $ticket->admin_profit);
        // dd($ticket->admin_profit);

        $ticket->issued_from = $request->input("issued_from");
        $ticket->ticket_status = $request->input("ticket_status");
        $ticket->payment_iata = $request->input("payment_iata");
        if ($ticket->save()) {
            return redirect()->back()->with('msg', 'Yor Booking Details Have Been Updated');
        } else {
    ...
    public function Select_Collectors_Store(request $req)
    {
        ...
        $collector_payment = CollectorPayment::where('collector_id', $req->collectorid)->first();

        if ($collector_payment) {

            $cashinhandamount = collectorcashinhand::firstOrCreate([
            ...
            $collector_payment->reason = $req->reason;
            $collector_payment->remaining_amount =   $cashinhandamount->total_cash_in_hand + $req->paid;
            ...
            $admin_collect->remaining_amount =   $cashinhandamount->total_cash_in_hand + $req->paid;
