...
class FlightControllerCopy extends Controller
{
    ...
    public function flightDataSingle($data, $airline, $aircarft)
    {
        ...
        foreach ($data as $flight) {
            ...
            $flights[$j]['class_type'] = $flight['travelerPricings'][0]['fareDetailsBySegment'][0]['class'];
            $i = 0;
    ...
    public function bookFlight(Request $request)
    {
        ...
        return view('flight-booking', ['data' => $data, 'payment' => $payment, 'error' => false]);
    }
    ...
    private function storeFlight(Request $request, $payment, $status, $data)
    {
        try {
            ...
            //            if (auth()->user()) {
            //                $request->user()->notify(new TicketProcessed($ticket));
            //            }
            //            else{
    ...
    public function download_Ticket($id)
    {
        ...
        if ($collector != null) {
            ...
            if ($ticket->format == 'db') {
                //$pdf = PDF::loadView('pdf2', ['data' => $data, 'ticket' => $ticket, 'passengers' => $passengers, 'collectorr' => $collectorr, 'pdf' => True])->setOptions(['defaultFont' => 'sans-serif'])->setPaper('A4', 'landscape');
                //$pdf = PDF::loadView('pdf_download', ['data' => $data, 'ticket' => $ticket, 'passengers' => $passengers, 'collectorr' => $collectorr, 'pdf' => True])->setOptions(['defaultFont' => 'sans-serif'])->setPaper('A4', 'landscape');
                $pdf = PDF::loadView('pdf_download',['data'=>$data,'ticket'=>$ticket,'passengers'=>$passengers,'collectorr'=>$collectorr,'pdf'=>True])->setOptions(['isHtml5ParserEnabled' => true, 'isRemoteEnabled' => true]);//->setWarnings(true);
                return $pdf->download('Ticket.pdf');
            } else {
                //$pdf = PDF::loadView('pdf', ['data' => $data, 'ticket' => $ticket, 'passengers' => $passengers, 'collectorr' => $collectorr, 'pdf' => True])->setOptions(['defaultFont' => 'sans-serif'])->setPaper('A4', 'landscape');
                //$pdf = PDF::loadView('pdf_download', ['data' => $data, 'ticket' => $ticket, 'passengers' => $passengers, 'collectorr' => $collectorr, 'pdf' => True])->setOptions(['defaultFont' => 'sans-serif'])->setPaper('A4', 'landscape');
